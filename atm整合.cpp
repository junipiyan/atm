...
void main1()
{
	printf("\n键入对应选项数字，按回车键确认。/Enter the option number and press Enter to confirm the option.\n\t\n");
	int a, b = 0;
	while (b != 1)
	{
		printf("\n\t1.用户请登录/Sign in\n");
		printf("\t2.用户请注册/Sign up\n");
		printf("\t0.退出/Exit\n");
		...
		if (FindACC(IV1))
		{
			while (n < 3)//3次输入错密码将退出系统/Exit system
			{
				...
				else
				{
					...
					if (n == 3)
					{
						printf("\n\t该账号已被冻结/The account has been frozen\n");
...
void ChangePhone()//修改电话
{
	...
	while (b < 3)
	{
		printf("\n\t请输入新联系电话号码/Please enter a new contact phone number:");
...
void SetPassword()//设置密码
{
	...
	{
		a++;
		printf("\n请设置你的账户密码/Please set your account password：\n");
		char storePassword[10] = { '\0' };
		getPassword(storePassword, 10);
...
void SetPhoneNum()//设置手机号
{
	...
	{
		a++;
		printf("请输入你的电话号码/Please enter your phone number：\n");
		scanf("%s", ZX.PhoneNum);
